            ...
            <v-col>
                ...
                <v-dialog v-model="dialog" max-width="650">
                    ...
                    <v-card-title>Evento</v-card-title>
                        <v-card-text>
                            <v-form ref="addEventbtn" @submit.prevent="addEvent()">
                                <v-row>
                                    <v-text-field v-model="name" type="text" label="Nombre del Evento" :rules="eventNameRules"></v-text-field>
                                </v-row>

                                <v-row>
                                    <v-text-field v-model="details" type="text" label="Detalle" :rules="eventDetailRules"></v-text-field>
                                </v-row>
                                <v-row>
                                    <v-col>
                                        <v-date-picker width="260" elevation="15" v-model="dates" range :color="color"></v-date-picker>
                                    </v-col>
                ...
                <v-sheet height="600">
                    <v-calendar ref="calendar" v-model="focus" color="primary" :events="events" :event-color="getEventColor" :event-margin-bottom="3" :now="today" :type="type" @click:event="showEvent" @click:more="viewDay" @click:date="setDialogDate" @change="updateRange"></v-calendar>
...
export default {
    data: () => ({
        ...
        eventDetailRules: [
            value => !!value || 'Campo Obligatorio',
            value => (value && value.length >= 10) || 'El Detalle debe contener más de 10 carácteres'
        ],
        eventDateRules: [
            value => !!value || 'Campo Obligatorio'
        ],
    ...
    methods: {
        ...
        async addEvent() {
            ...
            let valid = this.$refs.addEventbtn.validate();
